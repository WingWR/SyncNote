    ...
    <div class="bg-white rounded-lg p-6 w-96">
      ...
      <div class="space-y-3">
        <div>
          ...
          <input v-model.number="userId" type="number" placeholder="请输入用户ID" class="w-full px-3 py-2 border rounded-lg" />
        </div>
        <div>
      ...
      <div class="flex gap-2 justify-end mt-4">
        ...
        <button @click="addCollaboratorHandler" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">添加</button>
      </div>
    </div>
  </div>
</template>
...
const props = defineProps({ visible: Boolean, docId: Number })
...
async function addCollaboratorHandler() {
  ...
  try {
    await addCollaborator(props.docId, { userId: userId.value, permission: permission.value })
    const collaborators = await getCollaborators(props.docId)
